...
I think a lot of users would very much appreciate an addition to the `ggplot2`-verse that could handle classifying data according to the various styles offered by `classInt` (in effect wrapping `classInt` as does `tmap`).
