...
The starting point for me was the amazing visualisation of [earth.nullschool.net](https://earth.nullschool.net/#2023/10/26/0000Z/wind/surface/level/orthographic=-191.58,-45.04,2116).

Instead, I stumbled upon the [`rWind` package](https://github.com/jabiologo/rWind), which allows relatively easy download of [GFS (Global Forecast System)](https://www.ncei.noaa.gov/products/weather-climate-models/global-forecast) data. In combination with the [`rasterVis` package](https://oscarperpinan.github.io/rastervis/) I managed to cobble together something interesting to look at.

## Libraries
Some unfamiliar characters this time around. There's a lot of older formats involved in this particular toolchain hence some of the packages.
...
#| code-fold: true
#| results: false
nz <- st_read("~/Documents/teaching/Geog315/labs/nz.gpkg") %>%
  st_transform(4326) %>%
  as("Spatial")

w <- wind.dl(2023, 10, 26, 0, # year, month, day, hour
             165, 179,        # longitude range
             -48, -33)        # latitude range
...
I also densify the data a bit to get something a bit 'complete' seeming.
...
I don't entirely know, nor do I have time to get to the bottom of how to drive this function!
...
streamplot(wind_field, isField = TRUE,
           streamlet = list(L = 10), droplet = list(pc = 25),
           par.settings = streamTheme(symbol = brewer.pal(n = 9, 'YlOrRd'), cex = 0.5)) +
