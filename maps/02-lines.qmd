...
To be fair to `tmap`, it's using the _R_ graphics standard, which is that `lwd` is specified in pixels, which is notionally 1/96 inch, as [laid down by CSS](https://www.w3schools.com/cssref/css_units.php). This is 'device dependent' and when it comes to getting plots out of _R_ and into image file formats can lead to all kinds of headaches, but that's another story. A point is 1/72 inch, so `lwd = 1` gives you nominal line widths of 0.75 points in `tmap`.
