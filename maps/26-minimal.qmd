...
Anyway, to tackle this, I used the [`rmapshaper` package](http://www.mapshaper.org/) which can do topological simplification of polygons. You specify a `keep` parameter for the fraction of vertices to retain in the simplified geometries. For reasons of binary division, I started from 5.12% (the `ms_simplify` function defaults to 5%) and progressed in steps of 50%, all the way down to retaining 0.0025%. At 1 vertex in 40,000 or so of the original geometry we get quite a nice 27 vertex 'minimal' New Zealand. After that things start to go a bit pear-shaped (almost literally) as I show at the end of this page.
